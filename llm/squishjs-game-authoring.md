...
### Clicks / taps
Attach `onClick` to any visible node. Signature: **`(playerId, x, y)`** — the clicking player's id, and the click position in `0–100` plane space.
...
Nodes:        GameNode.Shape({ shapeType, coordinates2d, fill, border, onClick, playerIds })
              GameNode.Text({ textInfo:{ text,x,y,size,align,color,font }, playerIds })
              GameNode.Asset({ coordinates2d, assetInfo:{ key:{pos:{x,y},size:{x,y},startTime} }, playerIds })
